...
SQL migrations for the **match** service's own database (`betng_match`).
...
Only the match service writes here, and only the match service connects to
`betng_match`. Another service that needs this data asks for it over the match
