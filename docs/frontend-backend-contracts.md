...
## 7. Development stand-in

`packages/mock-data` implements every interface above for `VITE_DATA_SOURCE=mock` in development and test builds only, with each method annotated `@endpoint` for the route it stands in for. A deployed build never contains it (checked in `scripts/verify.sh`), and a mock-mode app shows a permanent "Development data" banner. Stand-in behaviours worth knowing when testing screens: deposits step `PENDING → PROCESSING → CONFIRMED` on successive status checks; amounts ending in 13 kobo fail and 17 kobo expire; account numbers starting `000` fail name enquiry; BVN/NIN starting `0` are rejected; KYC documents auto-review after ~20 s (a file name containing `blurry` is rejected); the TOTP code is `246810`; statements cannot be generated by the stand-in and report `FAILED`.
