...
describe("role → permission maps", () => {
  ...
  it("matches the maps the admin app was built against", () => {
    expect(ADMIN_ROLE_PERMISSIONS.OPERATIONS).toEqual([
      ...
      "audit:read", "health:read", "settings:read",
    ]);
    expect(ADMIN_ROLE_PERMISSIONS.RISK_ANALYST).toEqual([
      ...
      "reports:read", "health:read",
    ]);
    expect(ADMIN_ROLE_PERMISSIONS.SUPPORT).toEqual(["users:read", "shops:read", "audit:read", "health:read"]);
  });
  ...
  it("resolves shop roles", () => {
    expect(SHOP_ROLE_PERMISSIONS.CASHIER).toEqual(["tickets:sell", "tickets:check", "tickets:payout"]);
    expect(SHOP_ROLE_PERMISSIONS.MANAGER).toEqual([
      "tickets:sell", "tickets:check", "tickets:payout", "tickets:cancel", "transactions:read", "reports:read",
    ]);
