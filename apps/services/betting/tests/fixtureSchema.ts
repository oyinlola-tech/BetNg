...
// Tagged templates with no interpolation: static DDL that could only ever gain bound parameters, never spliced text.
export const FIXTURE_SCHEMA: readonly Sql[] = [
  sql`CREATE TABLE IF NOT EXISTS match.leagues (
  ...
  sql`CREATE TABLE IF NOT EXISTS match.teams (
  ...
  sql`CREATE TABLE IF NOT EXISTS match.fixtures (
  ...
  sql`CREATE TABLE IF NOT EXISTS odds.markets (
  ...
  sql`CREATE TABLE IF NOT EXISTS odds.market_selections (
  ...
  sql`GRANT SELECT ON ALL TABLES IN SCHEMA match TO betng_reader`,
  sql`GRANT SELECT ON ALL TABLES IN SCHEMA odds TO betng_reader`,
