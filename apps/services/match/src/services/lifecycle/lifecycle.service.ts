/**
 * The match lifecycle: every step of docs/architecture.md §5, one match at a time.
 *
 * The scheduler's tick and the admin actions both go through here, so a transition has one implementation
 * whoever asked for it. Each step is driven by database state and the clock, is safe to repeat, and marks
 * nothing done that did not happen: a peer that fails leaves the match where it was (or in the matching
 * `*_FAILED` state) with a backoff, and the next tick tries again.
 *
 * Nothing here reads a stake, a bettor or a selection, and nothing here can produce or alter a result: the
 * simulation is asked to run a match from its teams alone, and only the revealed part of what it committed is
 * copied onto the match row.
 */

import { randomUUID } from "node:crypto";
import { asId, ErrorCodes, teamStrengthSchema } from "@betng/contracts";
import type { LiveEventType, MatchLifecycle, TeamStrength } from "@betng/contracts";
...
import { MatchConflictError, PeerFailedError, ResultImmutableError } from "../../errors/index.js";
...
interface StepContext {
  ...
  /** An admin action reports a failed peer to its caller; the scheduler records it and moves on. */
...
export interface LifecycleService {
  ...
  voidMatch(matchId: string, actor: LifecycleActor, reason: string, requestId: string): Promise<void>;
}

export function createLifecycleService(deps: LifecycleDependencies): LifecycleService {
  const { catalogue, matches, lifecycle, simulation, betting, peers, timing, clock, logger } = deps;
  ...
  async function publishMilestone(match: MatchRecord, type: LiveEventType, description: string, requestId: string): Promise<void> {
  ...
  async function audit(
    ...
    match: MatchRecord,
    context: StepContext,
    detail: { readonly after?: unknown; readonly reason?: string; readonly severity?: "INFO" | "NOTICE" | "WARNING" | "CRITICAL" } = {},
  ): Promise<void> {
    try {
  ...
  async function ensureRounds(now: Date): Promise<void> {
    const leadMs = (timing.bettingCloseLeadSeconds + SCHEDULER.ROUND_MARGIN_SECONDS) * 1000;
    ...
    for (const league of await catalogue.listLeagues()) {
      ...
      while (upcoming < timing.upcomingRounds) {
        ...
        const earliest = Math.max(
          cursor === undefined ? 0 : cursor.kickoffAt.getTime() + timing.roundCycleSeconds * 1000,
          now.getTime() + leadMs,
        );
        const kickoffAt = new Date(alignToLeagueGrid(earliest, league.staggerSeconds, timing));
        ...
        const created = await matches.createFixtures(
          pairings.map((pairing) => ({ leagueId: league.id, ...next, ...pairing, kickoffAt, bettingClosesAt })),
  ...
  async function openOne(match: MatchRecord, context: StepContext): Promise<void> {
    const home = toTeamStrength(match.fixture.homeTeam);
    const away = toTeamStrength(match.fixture.awayTeam);

    try {
      await peers.odds.publishMarkets({ matchId: match.id, home, away }, context.requestId);
    } catch (error) {
      ...
      if (context.throwOnFailure) {
        throw new PeerFailedError(ErrorCodes.ODDS_UNAVAILABLE, "The odds service could not publish this match's markets.");
      }

      return;
    }

    const opened = await advance(match, "FIXTURE_CREATED", ["MARKETS_CREATED", "ODDS_PUBLISHED", "BETTING_OPEN"], context, {
    ...
    if (opened) {
      await publishMilestone(match, "BETTING_OPENED", `Betting is open: ${label(match)}.`, context.requestId);
    } else if (context.throwOnFailure) {
      throw new MatchConflictError("The match changed state while betting was being opened. Reload and try again.");
    }
  }

  async function publishMarkets(now: Date, context: StepContext): Promise<void> {
    ...
    for (const match of due) {
      if (await lifecycle.claim(match.id, "FIXTURE_CREATED", now, new Date(now.getTime() + SCHEDULER.LEASE_MS))) {
        await openOne(match, context);
      }
    }
    ...
    for (const match of missed) {
      ...
      if (closed) {
        await peers.odds.setMatchMarketsStatus(match.id, "CLOSED", context.requestId).catch(() => undefined);
      }
    }
  }

  async function markActive(now: Date, context: StepContext): Promise<void> {
    const open = await lifecycle.listDue({ states: ["BETTING_OPEN"], now, ignoreLease: true, limit: SCHEDULER.BATCH_SIZE });
    const withBets = new Set(await betting.matchesWithBets(open.map((match) => match.id)));
  ...
  async function closeOne(match: MatchRecord, context: StepContext): Promise<void> {
    const from = match.lifecycle as MatchLifecycle;

    try {
      await peers.odds.setMatchMarketsStatus(match.id, "CLOSED", context.requestId);
    } catch (error) {
      const reason = failureReason(ErrorCodes.ODDS_UNAVAILABLE, error);

      await lifecycle.recordFailure(match.id, from, reason, new Date(clock().getTime() + CLOSE_RETRY_MS));
      ...
      if (context.throwOnFailure) {
        throw new PeerFailedError(ErrorCodes.ODDS_UNAVAILABLE, "The odds service could not close this match's markets.");
      }
    ...
    if (closed) {
      await publishMilestone(match, "BETTING_CLOSED", `Betting is closed: ${label(match)}.`, context.requestId);
    } else if (context.throwOnFailure) {
      throw new MatchConflictError("The match changed state while betting was being closed. Reload and try again.");
    }
  }

  async function closeBettingStep(now: Date, context: StepContext): Promise<void> {
    const due = await lifecycle.listDue({
    ...
    for (const match of due) {
      ...
      if (await lifecycle.claim(match.id, state, now, new Date(now.getTime() + SCHEDULER.LEASE_MS))) {
        await closeOne(match, context);
      }
    }
  }
  ...
  async function runSimulation(match: MatchRecord, context: StepContext): Promise<void> {
    ...
    try {
      const run = await peers.simulation.runMatch(
        {
          ...
          home: {
            ...
            strength: pricedStrength(match.homeStrength) ?? toTeamStrength(homeTeam),
          },
          away: {
            teamId: asId<"TeamId">(awayTeam.id),
            name: awayTeam.name,
            shortName: awayTeam.shortName.slice(0, 8),
            strength: pricedStrength(match.awayStrength) ?? toTeamStrength(awayTeam),
          },
        },
        context.requestId,
      );
      ...
      if (run.status !== "COMPLETED" || run.matchId !== match.id) {
        throw new Error(`The simulation answered ${run.status} for match ${run.matchId}.`);
      }

      const published = await advance(match, "SIMULATION_STARTED", ["RESULT_GENERATED", "EVENTS_PUBLISHED"], context, {
        homeScore: 0,
        awayScore: 0,
        revealedSequence: 0,
        failureCount: 0,
      ...
      if (published) {
        // The entry names the run, never its outcome: audit logs are readable while the match is still in play.
      ...
      const reason = failureReason(ErrorCodes.SIMULATION_FAILED, error);
      ...
      logger.error("Simulation failed", {
        event: "match.simulationFailed",
        code: ErrorCodes.SIMULATION_FAILED,
        matchId: match.id,
        attempts,
        willRetry: attempts < SCHEDULER.MAX_SIMULATION_ATTEMPTS,
        requestId: context.requestId,
        reason,
      });
      await audit("simulation_failed", match, context, { reason, severity: "WARNING" });

      if (context.throwOnFailure) {
        throw new PeerFailedError(ErrorCodes.SIMULATION_FAILED, "The simulation service could not run this match.");
      }
    }
  }

  async function beginSimulation(match: MatchRecord, from: MatchLifecycle, context: StepContext): Promise<boolean> {
    const now = clock();
    const started = await advance(match, from, ["SIMULATION_STARTED"], context, {
      simulationStartedAt: now,
      nextAttemptAt: new Date(now.getTime() + SCHEDULER.LEASE_MS),
    });

    if (started && from === "BETTING_CLOSED") {
      await publishMilestone(match, "SIMULATION_STARTED", `Kick-off: ${label(match)}.`, context.requestId);
    }
  ...
  async function simulateStep(now: Date, context: StepContext): Promise<void> {
    ...
    for (const match of [...kickedOff, ...retries]) {
      if (await beginSimulation(match, match.lifecycle as MatchLifecycle, context)) {
        await runSimulation(match, context);
      }
    }
    ...
    const abandoned = await lifecycle.listDue({ states: ["SIMULATION_STARTED"], now, limit: SCHEDULER.BATCH_SIZE });

    for (const match of abandoned) {
      if (await lifecycle.claim(match.id, "SIMULATION_STARTED", now, new Date(now.getTime() + SCHEDULER.LEASE_MS))) {
        await runSimulation(match, context);
      }
    }
    ...
    const committed = new Set(await simulation.matchesWithResult(failed.map((match) => match.id)));
    ...
    for (const match of failed) {
      if (committed.has(match.id)) {
        await advance(
          ...
          context,
          { homeScore: 0, awayScore: 0, revealedSequence: 0, failureCount: 0, failureReason: null },
  ...
  async function revealOne(match: MatchRecord, now: Date, context: StepContext): Promise<number> {
  ...
  async function finishOne(match: MatchRecord, revealedSequence: number, context: StepContext): Promise<void> {
    if ((await simulation.countEventsAfter(match.id, revealedSequence)) > 0) return;
    ...
    const finished = await advance(match, "EVENTS_PUBLISHED", ["MATCH_FINISHED"], context, {
    ...
    if (finished) {
      await peers.odds.setMatchMarketsStatus(match.id, "SETTLED", context.requestId).catch((error: unknown) => {
  ...
  async function revealAndFinishStep(now: Date, context: StepContext): Promise<void> {
    ...
    for (const match of inPlay) {
      ...
      if (now.getTime() >= fullTimeMs(match.fixture.kickoffAt.getTime(), timing)) {
  ...
  async function runSettlement(match: MatchRecord, context: StepContext): Promise<void> {
    ...
    try {
      const outcome = await peers.settlement.settleMatch(match.id, context.requestId);
      ...
      const settled = await advance(match, "SETTLEMENT_STARTED", ["SETTLEMENT_COMPLETED"], context, {
        settledAt: clock(),
        failureCount: 0,
        failureReason: null,
      });
      ...
      if (settled) {
        await publishMilestone(match, "SETTLEMENT_COMPLETED", `Bets settled: ${label(match)}.`, context.requestId);
        await audit("settlement_completed", match, context, {
          after: { betsTotal: outcome.betsTotal, betsSettled: outcome.betsSettled, duplicate: outcome.duplicate },
        });
      }
    } catch (error) {
      const reason = failureReason(ErrorCodes.SETTLEMENT_FAILED, error);
      const attempts = match.failureCount + 1;
      ...
      await audit("settlement_failed", match, context, { reason, severity: "WARNING" });
    }
  }
  ...
  async function settleStep(now: Date, context: StepContext): Promise<void> {
    ...
    for (const match of due) {
      const from = match.lifecycle as MatchLifecycle;
      const started = await advance(match, from, ["SETTLEMENT_STARTED"], context, {
        nextAttemptAt: new Date(now.getTime() + SCHEDULER.LEASE_MS),
      });
      ...
      if (from === "MATCH_FINISHED") {
        await publishMilestone(match, "SETTLEMENT_STARTED", `Settling bets: ${label(match)}.`, context.requestId);
      }

      await runSettlement(match, context);
    }

    const abandoned = await lifecycle.listDue({ states: ["SETTLEMENT_STARTED"], now, limit: SCHEDULER.BATCH_SIZE });

    for (const match of abandoned) {
      if (await lifecycle.claim(match.id, "SETTLEMENT_STARTED", now, new Date(now.getTime() + SCHEDULER.LEASE_MS))) {
        await runSettlement(match, context);
      }
    }
  }

  async function runStep(name: string, requestId: string, step: () => Promise<void>): Promise<void> {
    try {
      await step();
    } catch (error) {
  ...
  async function requireMatch(matchId: string): Promise<MatchRecord> {
    ...
    if (match === undefined) throw new MatchConflictError("The match no longer exists.");

    return match;
  }

  function adminContext(actor: LifecycleActor, reason: string, requestId: string): StepContext {
  ...
  return {
    tick: async () => {
      ...
      const context: StepContext = { actor: SYSTEM_ACTOR, requestId, throwOnFailure: false };

      liveStreamDown = false;

      await runStep("ensureRounds", requestId, async () => ensureRounds(clock()));
      await runStep("publishMarkets", requestId, async () => publishMarkets(clock(), context));
      await runStep("markActive", requestId, async () => markActive(clock(), context));
      await runStep("closeBetting", requestId, async () => closeBettingStep(clock(), context));
      await runStep("simulate", requestId, async () => simulateStep(clock(), context));
      await runStep("revealAndFinish", requestId, async () => revealAndFinishStep(clock(), context));
      await runStep("settle", requestId, async () => settleStep(clock(), context));
    },
    ...
    openBetting: async (matchId, actor, reason, requestId) => {
      ...
      if (match.lifecycle !== "FIXTURE_CREATED") {
        throw new MatchConflictError(`Betting cannot be opened from ${match.lifecycle}.`);
      }

      if (match.fixture.bettingClosesAt.getTime() <= now.getTime()) {
        throw new MatchConflictError("This match's betting window has already passed.");
      }

      await lifecycle.resetAttempts(match.id, "FIXTURE_CREATED", now);

      if (!(await lifecycle.claim(match.id, "FIXTURE_CREATED", now, new Date(now.getTime() + SCHEDULER.LEASE_MS)))) {
        throw new MatchConflictError("The scheduler is opening this match right now. Reload and try again.");
      }
    ...
    closeBetting: async (matchId, actor, reason, requestId) => {
      ...
      if (match.lifecycle !== "BETTING_OPEN" && match.lifecycle !== "BETTING_ACTIVE") {
        throw new MatchConflictError(`Betting cannot be closed from ${match.lifecycle}.`);
      }
    ...
    startSimulation: async (matchId, actor, reason, requestId, mode) => {
      ...
      if (await simulation.hasResult(match.id)) throw new ResultImmutableError(match.id);

      const state = match.lifecycle as MatchLifecycle;
      const leaseLapsed = match.nextAttemptAt === null || match.nextAttemptAt.getTime() <= now.getTime();
      const legal =
      ...
      if (!legal) {
        throw new MatchConflictError(`The simulation cannot be ${mode === "START" ? "started" : "re-run"} from ${state}.`);
      }

      if (match.fixture.kickoffAt.getTime() > now.getTime()) {
        throw new MatchConflictError("This match has not reached its kick-off time.");
      }
      ...
      if (state === "SIMULATION_STARTED") {
        if (!(await lifecycle.claim(match.id, state, now, new Date(now.getTime() + SCHEDULER.LEASE_MS)))) {
          throw new MatchConflictError("This match is being simulated right now.");
        }
      } else {
        await lifecycle.resetAttempts(match.id, state, now);

        if (!(await beginSimulation(current, state, context))) {
          throw new MatchConflictError("The match changed state. Reload and try again.");
        }
      }
    ...
    voidMatch: async (matchId, actor, reason, requestId) => {
      ...
      if (match.lifecycle === "SETTLEMENT_COMPLETED" || match.lifecycle === "VOIDED") {
        throw new MatchConflictError(`A match cannot be voided from ${match.lifecycle}.`);
      }
      ...
      try {
        ...
        throw new PeerFailedError(ErrorCodes.ODDS_UNAVAILABLE, "The odds service could not void this match's markets.");
      }
      ...
      try {
        ...
        throw new PeerFailedError(ErrorCodes.SETTLEMENT_FAILED, "The settlement service could not refund this match's bets.");
      }

      const left = await lifecycle.voidMatch(match.id, actor.label, reason, clock());

      if (left === undefined) {
        throw new MatchConflictError("The match was settled or voided while this request was running.");
      }
      ...
      await audit("match_cancelled", match, context, {
        after: { lifecycle: "VOIDED", from: left, betsTotal: refund.betsTotal, betsRefunded: refund.betsSettled },
        reason,
        severity: "CRITICAL",
      });
    },
