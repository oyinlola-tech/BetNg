...
export class CreateFixtureHandler extends CommandHandler<
  ...
  public async execute(command: CreateFixtureCommand): Promise<AdminFixture> {
    ...
    for (const [path, teamId] of [
      ...
      if (
        ...
        throw new InvalidRequestError(
          "No active team with this id in the league.",
          path,
        );
      }
    }
    ...
    if (
      bettingClosesAt.getTime() <
      now.getTime() + SCHEDULER.ROUND_MARGIN_SECONDS * 1000
    ) {
      throw new InvalidRequestError(
        "Kick-off must leave time for betting to open and close.",
        "kickoffAt",
      );
    }
    ...
    const matchday =
      request.matchday ??
      (round !== undefined && round.season === season ? round.matchday : 1);
    ...
    try {
      ...
      const created =
        matchId === undefined ? undefined : await matches.findMatch(matchId);

      if (created === undefined)
        throw new MatchConflictError(
          "This pairing already exists in that matchday.",
        );
