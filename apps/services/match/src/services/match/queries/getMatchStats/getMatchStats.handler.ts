...
import { MatchNotFoundError, StatsNotAvailableError } from "../../../../errors/index.js";
import { liveStats, minuteAtMs, resultStatsSchema } from "../../../../utils/index.js";
import type { HandlerDependencies } from "../../match.dependencies.js";
import type { GetMatchStatsQuery } from "./getMatchStats.query.js";

export class GetMatchStatsHandler extends QueryHandler<GetMatchStatsQuery, MatchStats> {
  ...
  public async execute(query: GetMatchStatsQuery): Promise<MatchStats> {
    ...
    if (found.status !== "IN_PLAY" && found.status !== "COMPLETED") throw new StatsNotAvailableError(found.id);
    ...
    const minute = completed ? 90 : minuteAtMs(found.fixture.kickoffAt.getTime(), this.deps.clock().getTime(), this.deps.timing);
    const revealed =
