...
function requireAdmin(
  ...
  return {
    id: actor.id,
    role: actor.role,
    label: `admin:${actor.id}`.slice(0, 80),
...
export function createAdminController(
  ...
  return {
    ...
    createLeague: async (context) => {
      ...
      return commandBus.execute<CreateLeagueCommand, League>(
        new CreateLeagueCommand({ request, actor }),
      );
    },
    ...
    listTeams: async (context) => {
      ...
      return {
        items: await queryBus.execute<
          ListAdminTeamsQuery,
          readonly AdminTeam[]
        >(new ListAdminTeamsQuery(query.leagueId)),
      };
    },
    ...
    createTeam: async (context) => {
      ...
      return commandBus.execute<CreateTeamCommand, AdminTeam>(
        new CreateTeamCommand({ request, actor }),
      );
    },
    ...
    updateTeam: async (context) => {
      ...
      return commandBus.execute<UpdateTeamCommand, AdminTeam>(
        new UpdateTeamCommand({ teamId, request, actor }),
      );
    },
    ...
    listFixtures: async (context) => {
      ...
      return {
        ...
        >(
          new ListAdminFixturesQuery(
            parseQuery(context.query, adminFixturesQueryValidator),
          ),
        ),
      };
    },
    ...
    createFixture: async (context) => {
      ...
      return commandBus.execute<CreateFixtureCommand, AdminFixture>(
        new CreateFixtureCommand({ request, actor }),
      );
    },
    ...
    getMatch: async (context) => {
      ...
      return queryBus.execute<GetAdminMatchQuery, AdminMatchDto>(
        new GetAdminMatchQuery(idParam(context, "match")),
      );
    },
