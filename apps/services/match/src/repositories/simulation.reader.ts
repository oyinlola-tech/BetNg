  ...
  return {
    hasResult: async (matchId) =>
      orWhenTableMissing(async () => {
        const rows = await prisma.$queryRaw<{ found: number }[]>`
          SELECT 1 AS found FROM simulation.match_results WHERE match_id = ${matchId}::uuid LIMIT 1`;
    ...
    findResult: async (matchId) =>
      orWhenTableMissing<SimulationResultRow | undefined>(async () => {
        const rows = await prisma.$queryRaw<ResultColumns[]>`
          SELECT match_id::text AS match_id, home_goals::int AS home_goals, away_goals::int AS away_goals, stats
          FROM simulation.match_results
    ...
    listEvents: async (matchId, range) =>
      orWhenTableMissing(async () => {
        const rows = await prisma.$queryRaw<EventColumns[]>`
          SELECT id::text AS id, match_id::text AS match_id, sequence::int AS sequence, minute::int AS minute,
          ...
          WHERE match_id = ${matchId}::uuid
            AND sequence > ${range.after}
            AND sequence <= ${range.upTo ?? MAX_SEQUENCE}
    ...
    listScorers: async (leagueId, season, limit) =>
      orWhenTableMissing(async () => {
        ...
        const rows = await prisma.$queryRaw<ScorerColumns[]>`
          WITH revealed AS (
            ...
            WHERE f.league_id = ${leagueId}::uuid
              AND f.season = ${season}
              AND m.status::text IN ('IN_PLAY', 'COMPLETED')
              AND e.type::text = 'GOAL'
