...
describe("result secrecy", () => {
  ...
  it("has no score, events or stats before kick-off", async () => {
    ...
    expect(match.body).toMatchObject({
      status: "BETTING_OPEN",
      lifecycle: "BETTING_OPEN",
    });
    expect(match.body).not.toHaveProperty("score");
    expect((await get(`/matches/${fixture.matchId}/events`)).body).toEqual({
      items: [],
    });
  ...
  it("exposes only revealed information mid-match, to the public and to an admin", async () => {
    ...
    const events = (await get(`/matches/${fixture.matchId}/events`)).body as {
    ...
    expect(
      events.items.every((event) => matchEventSchema.safeParse(event).success),
    ).toBe(true);
    ...
    const listed = await get(
      `/admin/fixtures?leagueId=${fixture.leagueId}&matchday=1`,
      FIXTURE_ADMIN,
    );

    expect((listed.body as { items: unknown[] }).items).toEqual([
      expect.objectContaining({ score: { home: 1, away: 0 } }),
    ]);
    ...
    for (const answer of [
      ...
      expect(text).not.toMatch(
        /C\. Forward|D\. Holder|homeGoals|awayGoals|winner|winningGap/,
      );
      expect(text).not.toContain('"home":2');
    }

    expect((await get(`/results?leagueId=${fixture.leagueId}`)).body).toEqual({
      items: [],
    });
    expect(
      (
        (await get(`/leagues/${fixture.leagueId}/scorers`)).body as {
          items: unknown[];
        }
      ).items,
    ).toEqual([expect.objectContaining({ player: "A. Striker", goals: 1 })]);

    const table = (await get(`/leagues/${fixture.leagueId}/standings`))
      .body as { rows: { played: number }[] };
  ...
  it("publishes the full record once the match is finished", async () => {
    ...
    const results = (await get(`/results?leagueId=${fixture.leagueId}`))
    ...
    const scorers = (await get(`/leagues/${fixture.leagueId}/scorers`))
      .body as { items: unknown[] };

    expect(
      scorers.items.every((row) => topScorerSchema.safeParse(row).success),
    ).toBe(true);
    ...
    const recent = (
      await get(`/matches?status=COMPLETED&leagueId=${fixture.leagueId}`)
    ).body as { items: unknown[] };
...
describe("public routes", () => {
  it("lists leagues, teams with a code, fixtures and matches in the default window", async () => {
    ...
    const leagues = (await get("/leagues")).body as {
      items: { id: string; slug: string; status: string }[];
    };

    expect(
      leagues.items.find((league) => league.id === fixture.leagueId),
    ).toMatchObject({ status: "SUSPENDED", sport: "football" });

    const teams = (await get(`/teams?leagueId=${fixture.leagueId}`)).body as {
      items: unknown[];
    };
    ...
    const fixtures = (await get(`/fixtures?leagueId=${fixture.leagueId}`))
      .body as { items: unknown[] };

    expect(fixtures.items).toEqual([
    ...
    const later = new Date(
      fixture.kickoffAt.getTime() + 3_600_000,
    ).toISOString();

    expect(
      (
        (await get(`/fixtures?leagueId=${fixture.leagueId}&from=${later}`))
          .body as { items: unknown[] }
      ).items,
    ).toEqual([]);
    expect(
      (
        (await get(`/matches?leagueId=${fixture.leagueId}`)).body as {
          items: unknown[];
        }
      ).items,
    ).toHaveLength(1);
  ...
  it("rejects malformed input and unknown ids", async () => {
    ...
    expect(
      (await get("/matches/7f1c1f0e-5a53-4c58-9d0b-0d6f4d0a9b11")).status,
    ).toBe(404);
    expect(
      (await get("/leagues/7f1c1f0e-5a53-4c58-9d0b-0d6f4d0a9b11/standings"))
        .status,
    ).toBe(404);
  });
});
...
describe("admin routes", () => {
  it("enforces the actor kind, the permission and the internal token", async () => {
    ...
    expect(
      errorCode(
        (await get("/admin/teams", adminHeaders(["fixtures:read"]))).body,
      ),
    ).toBe("FORBIDDEN");
    expect(
      (
        await get("/admin/teams", {
          ...CATALOGUE_ADMIN,
    ...
    expect(
      (
        await get("/admin/teams", {
          ...CATALOGUE_ADMIN,
          "x-betng-internal-token": `${INTERNAL_TOKEN}x`,
        })
      ).status,
    ).toBe(401);
    expect(
    ...
    expect(
      (
        await send(
          "POST",
          "/admin/matches/7f1c1f0e-5a53-4c58-9d0b-0d6f4d0a9b11/actions",
          {},
          CATALOGUE_ADMIN,
        )
      ).status,
  ...
  it("creates a league, a team and a fixture", async () => {
    ...
    const league = await send(
      ...
      "/admin/leagues",
      {
        name: `Admin League ${tag}`,
        code: tag,
        slug: `admin-${tag.toLowerCase()}`,
        country: "Testland",
        status: "SUSPENDED",
      },
      CATALOGUE_ADMIN,
    ...
    expect(
      (
        await send(
          "POST",
          "/admin/teams",
          { leagueId, name: "Dup", shortName: "Dup", code: "AAA", ratings },
          CATALOGUE_ADMIN,
        )
      ).status,
    ).toBe(409);
    ...
    expect(
      new Date(kickoffAt).getTime() - fixture.bettingClosesAt.getTime(),
    ).toBe(10_000);
    expect(await lifecycleOf(harness, matchId)).toBe("FIXTURE_CREATED");

    const tooSoon = new Date(
      harness.clock.now().getTime() + 5000,
    ).toISOString();
    ...
    expect(
      (
        await send(
          "POST",
          "/admin/fixtures",
          { leagueId, homeTeamId: teams[0], awayTeamId: teams[0], kickoffAt },
          FIXTURE_ADMIN,
        )
      ).status,
    ).toBe(422);
  ...
  it("audits a ratings change and rolls it back when the audit cannot be written", async () => {
    ...
    expect(updated.body).toMatchObject({
      ratings: { attack: 90, midfield: 70 },
    });

    const audit = harness.peers.calls.audits.find(
      (entry) => entry.entityId === fixture.homeTeamId,
    );
    ...
    expect(
      (
        await harness.prisma.team.findUniqueOrThrow({
          where: { id: fixture.homeTeamId },
        })
      ).attack,
    ).toBe(90);
    expect(
      (await send("PATCH", path, { ratings: { attack: 120 } }, CATALOGUE_ADMIN))
        .status,
    ).toBe(422);
    expect(
      (await send("PATCH", path, { strength: 99 }, CATALOGUE_ADMIN)).status,
    ).toBe(422);
  });
  ...
  it("simulates a priced match on the ratings its odds were made from", async () => {
    ...
    await send(
      "PATCH",
      `/admin/teams/${fixture.homeTeamId}`,
      { ratings: { attack: 95 } },
      CATALOGUE_ADMIN,
    );
    harness.clock.set(fixture.bettingClosesAt);
    ...
    const request = harness.peers.calls.runMatch.find(
      (entry) => entry.matchId === fixture.matchId,
    );
  ...
  it("opens and closes betting on request, and refuses an illegal transition", async () => {
      ...
      errorCode(
        (
          await send(
            "POST",
            path,
            { action: "CLOSE_BETTING", reason: "too early" },
            FIXTURE_ADMIN,
          )
        ).body,
      ),
    ...
    const opened = await send(
      "POST",
      path,
      { action: "OPEN_BETTING", reason: "open it now" },
      FIXTURE_ADMIN,
    );
    ...
    expect(
      (
        await send(
          "POST",
          path,
          { action: "OPEN_BETTING", reason: "open it again" },
          FIXTURE_ADMIN,
        )
      ).status,
    ).toBe(409);
    ...
    const closed = await send(
      "POST",
      path,
      { action: "CLOSE_BETTING", reason: "close it early" },
      FIXTURE_ADMIN,
    );
      ...
      errorCode(
        (
          await send(
            "POST",
            path,
            { action: "START_SIMULATION", reason: "before kick-off" },
            FIXTURE_ADMIN,
          )
        ).body,
      ),
    ...
    expect(
      (
        await send(
          "POST",
          path,
          { action: "SET_SCORE", reason: "no such action" },
          FIXTURE_ADMIN,
        )
      ).status,
    ).toBe(422);
    expect(
      (await send("POST", path, { action: "VOID_MATCH" }, FIXTURE_ADMIN))
        .status,
    ).toBe(422);

    const transitions = await harness.prisma.matchTransition.findMany({
      where: { matchId: fixture.matchId },
      orderBy: { sequence: "asc" },
    });

    expect(transitions.at(-1)).toMatchObject({
      toState: "BETTING_CLOSED",
      actor: "admin:0b0f6a52-6f0e-4c53-9a53-1d1c6e0f7a11",
      reason: "close it early",
    });
  });
  ...
  it("re-runs a failed simulation, and never one that has a result", async () => {
    ...
    const path = `/admin/matches/${fixture.matchId}/actions`;
    const runs = (): number =>
      harness.peers.calls.runMatch.filter(
        (request) => request.matchId === fixture.matchId,
      ).length;
    ...
    const failedAgain = await send(
      "POST",
      path,
      { action: "RERUN_SIMULATION", reason: "retry now" },
      FIXTURE_ADMIN,
    );

    expect(failedAgain.status).toBe(502);
    expect(errorCode(failedAgain.body)).toBe("SIMULATION_FAILED");
    expect(JSON.stringify(failedAgain.body)).not.toContain(
      "simulation is down",
    );

    const rerun = await send(
      "POST",
      path,
      { action: "RERUN_SIMULATION", reason: "retry again" },
      FIXTURE_ADMIN,
    );
    ...
    for (const action of ["RERUN_SIMULATION", "START_SIMULATION"]) {
      const refused = await send(
        "POST",
        path,
        { action, reason: "try to re-simulate" },
        FIXTURE_ADMIN,
      );
  ...
  it("voids a match through settlement and audits it", async () => {
    ...
    const voided = await send(
      "POST",
      path,
      { action: "VOID_MATCH", reason: "fixture entered in error" },
      FIXTURE_ADMIN,
    );
    ...
    expect(harness.peers.calls.voidMatch).toContainEqual({
    ...
    expect(
      harness.peers.calls.audits.find(
        (entry) =>
          entry.entityId === fixture.matchId &&
          entry.action === "match_cancelled",
      ),
    ).toMatchObject({
      actorId: "0b0f6a52-6f0e-4c53-9a53-1d1c6e0f7a11",
      reason: "fixture entered in error",
    ...
    expect((await transitionsOf(harness, fixture.matchId)).at(-1)).toBe(
      "VOIDED",
    );
    expect(
      (
        await send(
          "POST",
          path,
          { action: "VOID_MATCH", reason: "void it twice" },
          FIXTURE_ADMIN,
        )
      ).status,
    ).toBe(409);
    ...
    expect(
      harness.peers.calls.runMatch.some(
        (request) => request.matchId === fixture.matchId,
      ),
    ).toBe(false);
  });
  ...
  it("does not void a settled match", async () => {
    ...
    const refused = await send(
      "POST",
      `/admin/matches/${fixture.matchId}/actions`,
      { action: "VOID_MATCH", reason: "too late now" },
      FIXTURE_ADMIN,
    );

    expect(refused.status).toBe(409);
    expect(
      harness.peers.calls.voidMatch.some(
  ...
  it("has no route that sets a score or a winner", async () => {
    const paths = harness.app.server.router
      .list()
    ...
    expect(
      paths.some(
        (route) =>
          /score|winner|result"/i.test(route) &&
          /"(POST|PUT|PATCH|DELETE)"/.test(route),
      ),
    ).toBe(false);
  });
});
