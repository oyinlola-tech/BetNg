...
export function EmptyState({ icon, title, description, action, compact = false, className }: EmptyStateProps): React.JSX.Element {
  return (
    <div className={cn("flex flex-col items-center justify-center text-center", compact ? "px-4 py-8" : "px-6 py-16", className)}>
      ...
      {description !== undefined && <p className="mt-1 max-w-xs text-sm text-text-muted">{description}</p>}
...
export function ErrorState({ error, onRetry, compact = false, className }: ErrorStateProps): React.JSX.Element {
  ...
  return (
    <div role="alert" className={cn("flex flex-col items-center justify-center text-center", compact ? "px-4 py-8" : "px-6 py-16", className)}>
      <div className="flex size-10 items-center justify-center rounded-md bg-danger-subtle text-danger">
        {offline ? <WifiOff className="size-5" /> : <RefreshCw className="size-5" />}
      </div>
      <p className="mt-3 text-md font-semibold text-text-primary">{presented.title}</p>
      <p className="mt-1 max-w-xs text-sm text-text-muted">{presented.message}</p>
