...
export function useMatches(
  filter: MatchFilter,
  options: { readonly enabled?: boolean; readonly refetchMs?: number } = {},
) {
  return useQuery({
...
export function useCompletedMatchdays(
  ...
  return useQuery({
    ...
    queryFn: () =>
      dataSource.listCompletedMatchdays(leagueId as LeagueId, season),
    enabled: leagueId !== undefined,
    refetchInterval: 15_000,
    placeholderData: (previous) => previous,
  });
...
export function useViewedMatches() {
  return useQuery({
    queryKey: queryKeys.viewed,
    queryFn: () => dataSource.listViewedMatches(),
  });
}
...
export function useSetPreferences() {
  ...
  return useMutation({
    mutationFn: (prefs: NotificationPreferences) =>
      dataSource.setNotificationPreferences(prefs),
    onSuccess: () => {
