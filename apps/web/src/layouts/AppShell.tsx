...
import { ConnectionStrip, ErrorBoundary, ErrorHelpProvider, PageSkeleton, useFlag, useIsDesktop, useLogger, useOnline, type ErrorHelpContextValue } from "@betng/ui-web";
import { AuthDialog, SessionGuard } from "../features/auth";
import { BetSlipBar, BetSlipPanel } from "../features/betslip";
import { SearchDialog } from "../features/search";
import { useAccountSync } from "../hooks/shellQueries";
import { useBetSignals } from "../hooks/useBetSignals";
...
import { useAppUpdates } from "./shell/useAppUpdates";

const ERROR_HELP: ErrorHelpContextValue = {
  hrefFor: (topic) => paths.help(topic),
...
function ConnectionStatus(): React.JSX.Element {
  ...
  const state = connection === "CONNECTING" ? "CONNECTED" : connection;

  if (!online) return <OfflineBanner online={false} />;
...
export function AppShell(): React.JSX.Element {
  ...
  return (
    <ErrorHelpProvider value={ERROR_HELP}>
