 ...
 *
 * `/health` is liveness: the process is up and serving. It never inspects a
 * dependency, so it never reports one as healthy.
 *
 * `/ready` is readiness: every dependency the service is actually configured
 * to use has been probed just now. A service with no configured dependencies
 * reports an empty `dependencies` list rather than inventing one.
 */
...
export interface DependencyCheck {
  /** The dependency's name, e.g. `postgres` or `match-service`. */
