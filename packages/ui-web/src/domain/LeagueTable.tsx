...
export function LeagueTable({
  ...
  return (
    <div className={cn("overflow-x-auto scrollbar-thin", className)}>
      <table className={cn("w-full text-sm", !compact && "min-w-[36rem]")}>
        <thead>
          <tr className="caps-label border-b border-border text-left">
            ...
            {!compact && (
              <th className="w-28 py-2 pr-3 text-right font-semibold">Form</th>
            )}
          </tr>
        </thead>
        <tbody>
          {standings.rows.map((row) => (
            <Row
              ...
              highlighted={highlightTeamIds.includes(row.team.id)}
            />
          ))}
        </tbody>
      </table>
    ...
    >
      <td className="py-2 pl-3">
        <span
          className={cn(
            ...
            top && "bg-brand-subtle text-brand",
            bottom && "bg-danger-subtle text-danger",
            !top && !bottom && "text-text-muted",
          )}
        >
          {row.position}
        </span>
      ...
      <td className="py-2">
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-xs font-medium hover:text-brand focus-ring"
        >
